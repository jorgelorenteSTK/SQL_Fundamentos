...
SELECT SUM(units_in_stock) AS quantity_stock FROM products;
...
SELECT MAX(unit_price) AS maximum, MIN(unit_price) AS minimum, AVG(unit_price) AS average FROM products;
...
SELECT COUNT(order_id) FROM orders WHERE employee_id IN (1, 3, 5, 6) GROUP BY employee_id;
...
SELECT ship_city, SUM(freight) AS sumatory_freight FROM orders WHERE ship_city IN('Madrid', 'Sevilla', 'Barcelona', 'Lisboa', 'London') GROUP BY ship_city ORDER BY sumatory_freight;

--obtener el precio promedio de los productos por categoria sin contar con los productos descontinuados (Discontinued)

SELECT AVG(unit_price) AS average_price FROM products WHERE discontinued = 0 GROUP BY category_id;
...
SELECT category_id, COUNT(product_id) FROM products WHERE unit_price BETWEEN 10 AND 60 GROUP BY category_id HAVING COUNT(product_id) > 12;
...
SELECT category_id, SUM(units_in_stock) AS sumatory FROM products WHERE supplier_id IN(17, 19, 16) GROUP BY category_id HAVING SUM(units_in_stock) < 100;
